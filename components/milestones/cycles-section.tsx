...
import { CalendarDays, Check, ChevronLeft, ChevronRight, Minus, Pencil, Plus, Repeat, Trash2 } from 'lucide-react';
import { toDateKey } from '@/lib/calendar';
import { formatRange, normaliseRange } from '@/lib/milestones';
import {
  CYCLE_KINDS,
  CYCLE_LABELS,
  CYCLE_SHORT_LABELS,
...
export function CyclesSection() {
  const { cycles, summary, today, hydrated } = useMilestoneCycles();
  ...
  return (
    <section className="mt-10 pb-16 md:mt-14 md:pb-24">
      ...
      {/* The week first: it is the thing that gets read daily, and the form
          below it is the thing that gets used once. */}
      {!hydrated ? (
        <div className="mb-6 h-48 animate-pulse rounded-2xl bg-foreground/5" aria-hidden />
      ) : (
        cycles.length > 0 && <WeekPreview cycles={cycles} today={today} />
      )}
 ...
 *
 * The cards below are each one milestone's own reckoning, which is the wrong
 * shape for the question actually asked most days — what is there to do on
 * Tuesday. Within a day the tasks stay grouped under the milestone they came
 * from, so "two runs and a pill" does not read as one undifferentiated list.
 *
 * Ticking here writes to the same place the cards do, so the two never disagree.
 */
function WeekPreview({ cycles, today }: { cycles: MilestoneCycle[]; today: string }) {
    ...
    <section className={`${cardClass} mb-6`}>
      ...
      {total === 0 ? (
        <p className="text-sm text-muted-foreground">
          Nothing falls in this week. The milestones below either have not started, have finished, or put
          their tasks on other days.
        </p>
      ) : (
          ...
          {day.groups.map((group) => (
            <div key={group.cycle.id}>
              ...
              <p
                ...
                className="mb-0.5 truncate text-[10px] uppercase tracking-wide text-muted-foreground"
              >
                {group.cycle.title}
              </p>
...
/**
 * The plus and minus beside a count. Smaller than the shared `IconButton`,
 * which is sized for the corner of a card rather than the end of a list row.
 */
function StepButton({
  title,
  onClick,
