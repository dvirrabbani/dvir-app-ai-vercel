 ...
 */
export function addDish(input: { name: string; parts?: DayPart[] }): Dish | null {
...
/** Renaming, or moving a dish to another stretch of the day. */
export function updateDish(id: string, changes: { name?: string; parts?: DayPart[] }): boolean {
  const menu = getMenu();
  ...
  writeMenu(menu.map((dish) => (dish.id === id ? { ...dish, name, parts } : dish)));
  return true;
}
